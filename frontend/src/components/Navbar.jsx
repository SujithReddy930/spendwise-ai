...
import {
  ...
  ScanText, History, Brain, TrendingUp, Sun, Moon, User
} from 'lucide-react'
    ...
    <>
      ...
      <aside className={`hidden md:flex fixed top-0 left-0 w-56 h-screen ${bg} border-r flex-col z-50`}>
        ...
        <div className={`p-3 border-t ${dark ? 'border-[#2a2a2a]' : 'border-gray-200'}`}>
          <Link to="/profile" className="flex items-center gap-2.5 px-3 py-2 mb-1 rounded-lg hover:bg-emerald-900/20 transition-colors">
      ...
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 h-16 ${bg} border-t flex items-center justify-around z-50 px-2`}>
        {[...mainLinks, ...aiLinks].slice(0, 5).map(link => {
          const Icon = link.icon
          const active = pathname === link.path
          return (
            <Link key={link.path} to={link.path} className={`flex flex-col items-center gap-1 px-2 py-1 rounded-lg ${active ? 'text-emerald-400' : textMuted}`}>
              <Icon size={20} />
